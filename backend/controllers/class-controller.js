...
const sclassCreate = async (req, res) => {
    try {
        ...
        if (existingSclassByName) {
            res.send({ message: 'Xin lỗi tên lớp này đã tồn tại' });
        }
        else {
...
const sclassList = async (req, res) => {
    try {
        ...
        if (sclasses.length > 0) {
            ...
            res.send({ message: "Không tìm thấy lớp học nào" });
        }
    } catch (err) {
        res.status(500).json(err);
    }
...
const deleteSclass = async (req, res) => {
    try {
        ...
        if (!deletedClass) {
            return res.send({ message: "Không tìm thấy lớp học" });
        }
...
const deleteSclasses = async (req, res) => {
    try {
        ...
        if (deletedClasses.deletedCount === 0) {
            return res.send({ message: "Không tìm thấy lớp nào để xóa" });
        }
